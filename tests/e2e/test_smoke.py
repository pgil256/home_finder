...
def test_S1_home_loads(client, base_url):
    ...
    assert "Pinellas" in r.text


def test_S2_scraper_form_loads(client, base_url):
    """Search form renders with city + property type fields."""
...
def test_S3_dashboard_renders_property_cards(client, base_url):
    """Dashboard returns 200 and renders at least one property card."""
    r = client.get(f"{base_url}/scraper/dashboard/", timeout=TIMEOUT)
    assert r.status_code == 200
...
def test_S5_invalid_parcel_returns_404(client, base_url):
    ...
    r = client.get(
        f"{base_url}/scraper/property/00-00-00-00000-000-0000/", timeout=TIMEOUT
    )
