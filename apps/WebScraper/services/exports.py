...
def _summarize_filters(request) -> list[tuple[str, str]]:
    ...
    if g.get("tax_status"):
        out.append(("Tax status", g["tax_status"]))
    if not out:
...
def generate_excel_response(request=None) -> HttpResponse:
    ...
    listings = qs_full.only(*[c[0] for c in LISTING_COLUMNS]).order_by(
        "-market_value"
    )[:MAX_EXCEL_PROPERTIES]
    ...
    for row_idx, prop in enumerate(listings, 2):
        for col_idx, (attr, _, fmt) in enumerate(LISTING_COLUMNS, 1):
            ...
            if value is not None and attr in (
                "market_value", "assessed_value", "tax_amount"
            ):
                value = float(value)
...
def generate_pdf_response(request=None) -> HttpResponse:
    ...
    listings = (
        qs_full.only(
            ...
            "assessed_value", "building_sqft", "year_built", "tax_amount",
    ...
    title_style = ParagraphStyle(
        "Title", fontSize=22, alignment=TA_CENTER, spaceAfter=4
    )
    subtitle_style = ParagraphStyle(
        "Subtitle", fontSize=11, alignment=TA_CENTER,
        textColor=rl_colors.grey, spaceAfter=18,
    )
    ...
    table = Table(
        data,
        colWidths=[2.3 * inch, 1.3 * inch, 1.7 * inch, 1.1 * inch,
                   0.7 * inch, 0.6 * inch, 1.0 * inch],
        repeatRows=1,
    )
    ...
    if len(prices):
        ...
        med = prices.median()
        mean = prices.mean()
        ax.axvline(med, color=chart_colors[2], linestyle="--", linewidth=2,
                   label=f"Median: {_money(med)}")
        ax.axvline(mean, color=chart_colors[3], linestyle="--", linewidth=2,
                   label=f"Mean:   {_money(mean)}")
        ax.set_title(
    ...
    if len(plot_df) > 5:
        ...
        ax.scatter(plot_df["building_sqft"], plot_df["market_value"],
                   c=chart_colors[1], alpha=0.4, s=20, edgecolors="none")
        # trend line
        if len(plot_df) > 10:
            ...
            ax.plot(xs, np.poly1d(z)(xs), color=chart_colors[2], linestyle="--",
                    linewidth=2, label=f"~ ${z[0]:,.0f} per sqft")
            ax.legend()
        ax.set_title(
    ...
    if len(years):
        fig, ax = plt.subplots(figsize=(11, 5.5))
        ax.hist(years, bins=range(int(years.min()), int(years.max()) + 5, 5),
                color=chart_colors[3], edgecolor="white", alpha=0.85)
        ax.axvline(years.median(), color=chart_colors[0], linestyle="--",
                   linewidth=2, label=f"Median year: {int(years.median())}")
        ax.set_title("Year built distribution (5-year buckets)",
                     fontsize=13, fontweight="bold", pad=12)
    ...
    if show_city_chart and "city" in df.columns:
        ...
        if len(city_stats):
            fig, ax = plt.subplots(figsize=(11, 5.5))
            bars = ax.bar(range(len(city_stats)), city_stats["mean"],
            ...
            ax.set_title("Mean market value by city (cities with ≥ 50 matches)",
                         fontsize=13, fontweight="bold", pad=12)
