...
def mock_time_sleep():
    ...
    with (
        patch('apps.analytics.tasks.pcpao_scraper.time.sleep'),
        patch('apps.analytics.tasks.tax_collector_scraper.time.sleep'),
    ):
        yield
...
def mock_chrome_dependencies():
    ...
    with (
        patch('apps.analytics.tasks.pcpao_scraper.os.path.exists', return_value=False),
        patch('apps.analytics.tasks.tax_collector_scraper.os.path.exists', return_value=False),
        patch('apps.analytics.tasks.pcpao_scraper.ChromeDriverManager') as mock_cdm1,
        patch('apps.analytics.tasks.tax_collector_scraper.ChromeDriverManager') as mock_cdm2,
    ):
...
class TestPCPAOScraper:
    ...
    def mock_webdriver(self):
        ...
        with patch('apps.analytics.tasks.pcpao_scraper.webdriver') as mock_wd:
    ...
    def mock_chrome_paths(self):
        ...
        with patch('apps.analytics.tasks.pcpao_scraper.os.path.exists', return_value=False):
            yield
    ...
    def test_scraper_initialization(self, mock_webdriver, mock_chrome_paths):
        ...
        from apps.analytics.tasks.pcpao_scraper import PCPAOScraper
    ...
    def test_setup_driver_creates_chrome_instance(self, mock_chrome_dependencies):
        ...
        import apps.analytics.tasks.pcpao_scraper as scraper_module
        from apps.analytics.tasks.pcpao_scraper import PCPAOScraper
...
class TestTaxCollectorScraper:
    ...
    def mock_webdriver(self):
        """Create mock Chrome WebDriver."""
        with patch('apps.analytics.tasks.tax_collector_scraper.webdriver') as mock_wd:
            mock_driver = MagicMock()
            mock_wd.Chrome.return_value = mock_driver
            yield mock_driver
    ...
    def mock_chrome_paths(self):
        """Mock Chrome binary path checks."""
        with patch('apps.analytics.tasks.tax_collector_scraper.os.path.exists', return_value=False):
            yield

    def test_scraper_initialization(self, mock_webdriver, mock_chrome_paths):
        """Test scraper initializes with correct default settings."""
        from apps.analytics.tasks.tax_collector_scraper import TaxCollectorScraper
    ...
    def test_setup_driver_creates_chrome_instance(self, mock_chrome_dependencies):
        """Test setup_driver creates Chrome WebDriver."""
        import apps.analytics.tasks.tax_collector_scraper as scraper_module
        from apps.analytics.tasks.tax_collector_scraper import TaxCollectorScraper
